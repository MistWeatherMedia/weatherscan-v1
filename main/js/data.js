...
async function getCoreData() {
  ...
  async function getMoons() {
        ...
        for (var i = 0; i < weatherData.almanac.moonphases.length; i++) {
...
async function getExtraCore(locNum) {
  async function getBulletinAlerts() {
...
async function getSpanishData() {
  async function getCurrent() {
    ...
    try {const data = await $.getJSON(url)
...
async function getHealthData() {
  ...
  async function getAchesPains() {
    weatherData.achesBreathe.dayName = ""
    ...
    try {
      ...
      weatherData.achesBreathe.dayName = weatherData.achesBreathe.dayName == "" ? dateFns.format(new Date(breathdata.breathingIndex12hour.fcstValidLocal[ii]), "dddd") : weatherData.achesBreathe.dayName
    } catch (error) {
      console.error(error)
...
async function allData() {
  if ((systemSettings.packageSettings.includes("forecast") || systemSettings.packageSettings.includes("minicoreone") || systemSettings.packageSettings.includes("minicoretwo")) && !systemSettings.appearanceSettings.nationalConfig) {
    await getCoreData()
  }
...
async function startPrograms() {
  ...
  setTimeout(() => {
    ...
    $('#startup').fadeOut(0)
  }, 5000);//wait a few seconds bcuz i like watching the weatherscan logo slide in the startup screen 
}
